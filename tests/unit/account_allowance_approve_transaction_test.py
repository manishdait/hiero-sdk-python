...
def test_approve_token_nft_allowance_all_serials(account_allowance_transaction, sample_accounts, sample_tokens):
    ...
    result = account_allowance_transaction.approve_token_nft_allowance_all_serials(token_id, owner, spender)
...
def test_delete_token_nft_allowance_all_serials(account_allowance_transaction, sample_accounts, sample_tokens):
    """Test deleting NFT allowance for all serials"""
    token_id = sample_tokens["token1"]
    owner = sample_accounts["owner"]
    spender = sample_accounts["spender"]

    result = account_allowance_transaction.delete_token_nft_allowance_all_serials(token_id, owner, spender)
...
def test_build_proto_body_with_allowances(account_allowance_transaction, sample_accounts, sample_tokens):
...
def test_require_not_frozen(account_allowance_transaction, sample_accounts):
    ...
    with pytest.raises(Exception, match="Transaction is immutable"):
        account_allowance_transaction.approve_token_allowance(TokenId(0, 0, 100), owner, spender, 1000)

    with pytest.raises(Exception, match="Transaction is immutable"):
        account_allowance_transaction.approve_token_nft_allowance(NftId(TokenId(0, 0, 100), 1), owner, spender)
...
def test_mixed_allowance_types(account_allowance_transaction, sample_accounts, sample_tokens):
    ...
    account_allowance_transaction.approve_token_allowance(token_id, owner, spender, 1000)
    account_allowance_transaction.approve_token_nft_allowance(nft_id, owner, spender)
    account_allowance_transaction.approve_token_nft_allowance_all_serials(sample_tokens["token2"], owner, spender)
