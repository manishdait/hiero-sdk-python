...
def test_build_transaction_body(mock_account_ids):
    ...
    update_tx = (
        TokenUpdateNftsTransaction().set_token_id(token_id).set_serial_numbers(serial_numbers).set_metadata(metadata)
    )
...
def test_constructor_with_parameters(mock_account_ids):
    ...
    update_tx = TokenUpdateNftsTransaction(token_id=token_id, serial_numbers=serial_numbers, metadata=metadata)
...
def test_update_nfts_transaction_from_proto(mock_account_ids):
    ...
    proto = TokenUpdateNftsTransactionBody(
        token=token_id._to_proto(), serial_numbers=serial_numbers, metadata=BytesValue(value=metadata_bytes)
    )
