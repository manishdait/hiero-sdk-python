...
def test_build_transaction_body(mock_account_ids):
    ...
    grant_kyc_tx = TokenGrantKycTransaction().set_token_id(token_id).set_account_id(account_id)
...
def test_grant_kyc_transaction_can_execute(mock_account_ids):
    ...
    with mock_hedera_servers(response_sequences) as client:
        transaction = TokenGrantKycTransaction().set_token_id(token_id).set_account_id(account_id)
...
def test_grant_kyc_transaction_from_proto(mock_account_ids):
    ...
    proto = TokenGrantKycTransactionBody(token=token_id._to_proto(), account=account_id._to_proto())
