...
def test_from_string_for_invalid_format(invalid_id):
    ...
    with pytest.raises(
        ValueError,
        match=f"Invalid file ID string '{invalid_id}'. Expected format 'shard.realm.file'.",
    ):
...
def test_from_proto():
    ...
    proto = basic_types_pb2.FileID(shardNum=1, realmNum=2, fileNum=3)
...
def test_from_proto_zero_values():
    ...
    proto = basic_types_pb2.FileID(shardNum=0, realmNum=0, fileNum=0)
