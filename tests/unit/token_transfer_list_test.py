...
def test_token_transfer_list_constructor(mock_account_ids):
    ...
    assert token_transfer_list.expected_decimals is None
    assert token_transfer_list.transfers == []
    assert token_transfer_list.nft_transfers == []

    # Test with explicit expected_decimals
    decimal_token_transfer_list = TokenTransferList(token=token_id, expected_decimals=expected_decimals)
    ...
    transfers_token_transfer_list = TokenTransferList(token=token_id, transfers=transfers)
    assert transfers_token_transfer_list.token == token_id
    assert transfers_token_transfer_list.expected_decimals is None
    ...
    nft_transfers_token_transfer_list = TokenTransferList(token=token_id, nft_transfers=nft_transfers)
    assert nft_transfers_token_transfer_list.token == token_id
    assert nft_transfers_token_transfer_list.expected_decimals is None
...
def test_to_proto(mock_account_ids):
    ...
    token_transfer_list = TokenTransferList(token=token_id_1, transfers=transfers, expected_decimals=expected_decimals)
