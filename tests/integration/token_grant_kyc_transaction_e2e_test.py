...
def test_token_grant_kyc_transaction_fails_with_no_kyc_key():
    ...
    try:
        ...
        assert receipt.status == ResponseCode.TOKEN_HAS_NO_KYC_KEY, (
            f"Token grant KYC should have failed with TOKEN_HAS_NO_KYC_KEY status but got: {ResponseCode(receipt.status).name}"
        )
...
def test_token_grant_kyc_transaction_fails_when_account_not_associated():
    ...
    try:
        ...
        assert receipt.status == ResponseCode.TOKEN_NOT_ASSOCIATED_TO_ACCOUNT, (
            f"Token grant KYC should have failed with TOKEN_NOT_ASSOCIATED_TO_ACCOUNT status but got: {ResponseCode(receipt.status).name}"
        )
    finally:
        env.close()
