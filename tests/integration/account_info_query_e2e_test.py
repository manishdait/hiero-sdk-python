...
def test_integration_account_info_query_can_execute():
    ...
    try:
        ...
        assert str(info.account_id) == str(new_account_id), (
            f"Expected account ID {new_account_id}, but got {info.account_id}"
        )
        assert info.balance.to_tinybars() == Hbar(1).to_tinybars(), (
            f"Expected balance of 1 Hbar, but got {info.balance}"
        )
        assert info.key.to_bytes_raw() == new_account_public_key.to_bytes_raw(), (
            f"Expected public key {new_account_public_key}, but got {info.key}"
        )
        assert info.receiver_signature_required == False, "Expected receiver signature to not be required, but it was"
        assert info.auto_renew_period == Duration(7890000), (
            f"Expected auto renew period of 7890000 seconds, but got {info.auto_renew_period}"
        )
        assert info.expiration_time is not None, "Expected expiration time to be set, but it was None"
        assert info.account_memo == account_memo, (
            f"Expected account memo '{account_memo}', but got '{info.account_memo}'"
        )
        ...
        assert info.proxy_received.to_tinybars() == 0, (
            f"Expected 0 proxy received tinybars, but got {info.proxy_received.to_tinybars()}"
        )
    finally:
        env.close()
...
def test_integration_account_info_query_token_relationship_info():
    ...
    try:
        ...
        token_id = create_fungible_token(
            env, [lambda tx: tx.set_kyc_key(env.operator_key), lambda tx: tx.set_freeze_default(False)]
        )  # Must not be frozen for token operations
        ...
        assert receipt.status == ResponseCode.SUCCESS, (
            f"Token associate failed with status: {ResponseCode(receipt.status).name}"
        )

        # Get account info and verify token relationship
        info = AccountInfoQuery(new_account.id).execute(env.client)

        assert len(info.token_relationships) == 1, (
            f"Expected 1 token relationship, but got {len(info.token_relationships)}"
        )
        ...
        assert relationship.freeze_status == TokenFreezeStatus.UNFROZEN, (
            f"Expected freeze status to be UNFROZEN, but got {relationship.freeze_status}"
        )
        assert relationship.kyc_status == TokenKycStatus.REVOKED, (
            f"Expected KYC status to be REVOKED, but got {relationship.kyc_status}"
        )
        ...
        assert relationship.automatic_association == False, (
            f"Expected automatic association to be False, but got {relationship.automatic_association}"
        )

        # Unfreeze account for token
        receipt = TokenUnfreezeTransaction().set_account_id(new_account.id).set_token_id(token_id).execute(env.client)
        assert receipt.status == ResponseCode.SUCCESS, (
            f"Token freeze failed with status: {ResponseCode(receipt.status).name}"
        )

        # Grant KYC to account
        receipt = TokenGrantKycTransaction().set_account_id(new_account.id).set_token_id(token_id).execute(env.client)
        assert receipt.status == ResponseCode.SUCCESS, (
        ...
        assert relationship.kyc_status == TokenKycStatus.GRANTED, (
            f"Expected KYC status to be GRANTED, but got {relationship.kyc_status}"
        )
    finally:
        env.close()
...
def test_integration_account_info_query_token_relationships_length():
    ...
    try:
        ...
        decimals_token_id = create_fungible_token(
            env, [lambda tx: tx.set_decimals(8), lambda tx: tx.set_kyc_key(env.operator_key)]
        )
        receipt = (
            TokenAssociateTransaction()
            .set_account_id(new_account.id)
        ...
        assert info.token_relationships[0].decimals == 8, (
            f"Expected decimals to be 8, but got {info.token_relationships[0].decimals}"
        )
        ...
        assert len(info.token_relationships) == 2, (
            f"Expected 2 token relationships, but got {len(info.token_relationships)}"
        )
        assert info.token_relationships[0].decimals == 2, (
            f"Expected decimals to be 2, but got {info.token_relationships[0].decimals}"
        )
        assert info.token_relationships[1].decimals == 8, (
            f"Expected decimals to be 8, but got {info.token_relationships[1].decimals}"
        )
        ...
        assert receipt.status == ResponseCode.SUCCESS, (
            f"Token dissociate failed with status: {ResponseCode(receipt.status).name}"
        )

        # Check account info has no token relationships
        info = AccountInfoQuery(new_account.id).execute(env.client)
        assert str(info.account_id) == str(new_account.id), (
            f"Expected account ID {new_account.id}, but got {info.account_id}"
        )
        assert len(info.token_relationships) == 0, (
            f"Expected 0 token relationships, but got {len(info.token_relationships)}"
        )
    finally:
        env.close()
...
def test_integration_account_info_query_nft_owned():
    ...
    try:
        ...
        assert receipt.status == ResponseCode.SUCCESS, (
            f"Token mint failed with status: {ResponseCode(receipt.status).name}"
        )
