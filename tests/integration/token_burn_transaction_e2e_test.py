...
def test_integration_token_burn_transaction_can_execute():
    ...
    try:
        ...
        receipt = TokenBurnTransaction().set_token_id(token_id).set_amount(10).execute(env.client)

        assert receipt.status == ResponseCode.SUCCESS, (
            f"Token burn failed with status: {ResponseCode(receipt.status).name}"
        )
...
def test_integration_token_burn_transaction_no_amount():
    ...
    try:
        ...
        receipt = TokenBurnTransaction().set_token_id(token_id).execute(env.client)
...
def test_integration_token_burn_transaction_nft():
    ...
    try:
        ...
        # Mint an NFT
        receipt = TokenMintTransaction().set_token_id(token_id).set_metadata([b"Metadata"]).execute(env.client)
        assert receipt.status == ResponseCode.SUCCESS, (
            f"Token mint failed with status: {ResponseCode(receipt.status).name}"
        )

        # Try to burn the NFT
        receipt = TokenBurnTransaction().set_token_id(token_id).set_serials([1]).execute(env.client)
        assert receipt.status == ResponseCode.SUCCESS, (
            f"Token burn failed with status: {ResponseCode(receipt.status).name}"
        )
...
def test_integration_token_burn_transaction_fails_invalid_metadata():
    ...
    try:
        ...
        receipt = TokenBurnTransaction().set_token_id(token_id).set_amount(1).execute(env.client)
        assert receipt.status == ResponseCode.INVALID_TOKEN_BURN_METADATA, (
            f"Token burn should have failed with status: {ResponseCode(receipt.status).name}"
        )
    finally:
        env.close()
...
def test_integration_token_burn_transaction_fails_no_supply_key():
    ...
    try:
        ...
        receipt = TokenBurnTransaction().set_token_id(token_id).set_amount(10).execute(env.client)
        assert receipt.status == ResponseCode.TOKEN_HAS_NO_SUPPLY_KEY, (
            f"Token burn should have failed with TOKEN_HAS_NO_SUPPLY_KEY but got: {ResponseCode(receipt.status).name}"
        )
    finally:
        env.close()
