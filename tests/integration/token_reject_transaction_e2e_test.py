...
def test_integration_token_reject_transaction_can_execute():
    ...
    try:
        ...
        token_associate_transaction = TokenAssociateTransaction(account_id=account_id, token_ids=[token1, token2])
        ...
        # Reject the tokens
        token_reject_transaction = TokenRejectTransaction(owner_id=account_id, token_ids=[token1, token2])
        ...
        assert (
            balance.token_balances is not None
            and balance.token_balances.get(token1) == 0
            and balance.token_balances.get(token2) == 0
        )
        ...
        assert (
            balance.token_balances is not None
            and balance.token_balances.get(token1) == 1000
            and balance.token_balances.get(token2) == 1000
        )
    finally:
        env.close()
...
def test_integration_token_reject_transaction_can_execute_for_nft():
    ...
    try:
        ...
        receipt = (
            TokenMintTransaction().set_token_id(nft_id_1).set_metadata([b"metadata1", b"metadata2"]).execute(env.client)
        )
        assert receipt.status == ResponseCode.SUCCESS, (
            f"NFT minting failed with status: {ResponseCode(receipt.status).name}"
        )
        serials_1 = receipt.serial_numbers

        receipt = (
            TokenMintTransaction().set_token_id(nft_id_2).set_metadata([b"metadata1", b"metadata2"]).execute(env.client)
        )
        assert receipt.status == ResponseCode.SUCCESS, (
            f"NFT minting failed with status: {ResponseCode(receipt.status).name}"
        )
        ...
        receipt = (
            ...
            .set_account_memo("Receiver Account")
            .execute(env.client)
        )
        assert receipt.status == ResponseCode.SUCCESS, (
            f"Account creation failed with status: {ResponseCode(receipt.status).name}"
        ...
        balance = CryptoGetAccountBalanceQuery(account_id).execute(env.client)
        assert balance.token_balances and balance.token_balances.get(nft_id_1) == 1, (
            f"Expected 1 NFT for token {nft_id_1}, got {balance.token_balances.get(nft_id_1)}"
        )
        assert balance.token_balances and balance.token_balances.get(nft_id_2) == 1, (
            f"Expected 1 NFT for token {nft_id_2}, got {balance.token_balances.get(nft_id_2)}"
        )
        ...
        assert nft_info_1.account_id == env.operator_id, (
            f"Expected NFT owner to be {env.operator_id}, got {nft_info_1.account_id}"
        )

        nft_info_2 = TokenNftInfoQuery(NftId(nft_id_2, serials_2[1])).execute(env.client)
        assert nft_info_2.account_id == env.operator_id, (
            f"Expected NFT owner to be {env.operator_id}, got {nft_info_2.account_id}"
        )
    finally:
        env.close()
...
def test_integration_token_reject_transaction_can_execute_for_ft_and_nft_parallel():
    ...
    try:
        ...
        receipt = (
            TransferTransaction()
            .add_token_transfer(token_id_1, account_id, 10)
            .add_token_transfer(token_id_1, env.client.operator_account_id, -10)
            .add_token_transfer(token_id_2, account_id, 10)
            .add_token_transfer(token_id_2, env.client.operator_account_id, -10)
            .add_nft_transfer(NftId(nft_id_1, serials_1[0]), env.client.operator_account_id, account_id)
            .add_nft_transfer(NftId(nft_id_1, serials_1[1]), env.client.operator_account_id, account_id)
            .add_nft_transfer(NftId(nft_id_2, serials_2[0]), env.client.operator_account_id, account_id)
            .add_nft_transfer(NftId(nft_id_2, serials_2[1]), env.client.operator_account_id, account_id)
        ...
        balance = CryptoGetAccountBalanceQuery(account_id).execute(env.client)
        assert balance.token_balances and balance.token_balances.get(token_id_1) == 0, (
            f"Expected token balance to be 0 for token {token_id_1}, got {balance.token_balances.get(token_id_1)}"
        )
        assert balance.token_balances and balance.token_balances.get(token_id_2) == 0, (
            f"Expected token balance to be 0 for token {token_id_2}, got {balance.token_balances.get(token_id_2)}"
        )

        balance = CryptoGetAccountBalanceQuery(env.operator_id).execute(env.client)
        assert balance.token_balances and balance.token_balances.get(token_id_1) == 1000, (
            f"Expected token balance to be 1000 for token {token_id_1}, got {balance.token_balances.get(token_id_1)}"
        )
        assert balance.token_balances and balance.token_balances.get(token_id_2) == 1000, (
            f"Expected token balance to be 1000 for token {token_id_2}, got {balance.token_balances.get(token_id_2)}"
        )
...
def test_token_reject_transaction_fails_with_invalid_signature():
    ...
    try:
        ...
        # Associate the tokens to the new account
        token_associate_transaction = TokenAssociateTransaction(account_id=account_id, token_ids=[token1])
        ...
        # Reject the tokens
        token_reject_transaction = TokenRejectTransaction(owner_id=account_id, token_ids=[token1])
        ...
        assert receipt.status == ResponseCode.INVALID_SIGNATURE, (
            f"Token rejection should have failed with INVALID_SIGNATURE status but got: {ResponseCode(receipt.status).name}"
        )
    finally:
        env.close()
...
def test_integration_token_reject_transaction_fails_with_reference_size_exceeded():
    ...
    try:
        ...
        receipt = (
            ...
            .set_token_id(nft_id)
            .set_metadata([b"1", b"2", b"3", b"4", b"5", b"6", b"7", b"8", b"9", b"10"])
            .execute(env.client)
        )
        assert receipt.status == ResponseCode.SUCCESS, (
            f"NFT minting failed with status: {ResponseCode(receipt.status).name}"
        ...
        account_transaction = AccountCreateTransaction(
            key=new_account_public_key, initial_balance=Hbar(1), memo="Receiver Account"
        )
        ...
        # Associate the tokens to the new account
        token_associate_transaction = TokenAssociateTransaction(account_id=account_id, token_ids=[nft_id, token_id])
        ...
        receipt = (
            TransferTransaction()
            .add_token_transfer(token_id, env.client.operator_account_id, -10)
            .add_token_transfer(token_id, account_id, 10)
            .add_nft_transfer(NftId(nft_id, serials[0]), env.client.operator_account_id, account_id)
        ...
        token_reject_transaction = (
            ...
            .set_nft_ids(
                [
                    NftId(nft_id, serials[0]),
                    NftId(nft_id, serials[1]),
                    NftId(nft_id, serials[2]),
                    NftId(nft_id, serials[3]),
                    NftId(nft_id, serials[4]),
                    NftId(nft_id, serials[5]),
                    NftId(nft_id, serials[6]),
                    NftId(nft_id, serials[7]),
                    NftId(nft_id, serials[8]),
                    NftId(nft_id, serials[9]),
                ]
            )
        )

        token_reject_transaction.transaction_fee = Hbar(
            3
        ).to_tinybars()  # Set transaction fee to be higher than 2 Hbars
        receipt = token_reject_transaction.freeze_with(env.client).sign(new_account_private_key).execute(env.client)
        assert receipt.status == ResponseCode.TOKEN_REFERENCE_LIST_SIZE_LIMIT_EXCEEDED, (
            f"Token rejection should have failed with TOKEN_REFERENCE_LIST_SIZE_LIMIT_EXCEEDED status but got: {ResponseCode(receipt.status).name}"
        )
    finally:
        env.close()
...
def test_integration_token_reject_transaction_fails_treasury_rejects():
    ...
    try:
        ...
        token_reject_transaction = TokenRejectTransaction(owner_id=env.operator_id, token_ids=[token_id])

        receipt = token_reject_transaction.execute(env.client)

        assert receipt.status == ResponseCode.ACCOUNT_IS_TREASURY, (
            f"Token rejection should have failed with ACCOUNT_IS_TREASURY status but got: {ResponseCode(receipt.status).name}"
        )
        ...
        # Mint NFT
        mint = TokenMintTransaction(token_id=nft_token_id, metadata=[b"test metadata"])
        ...
        token_reject_transaction = TokenRejectTransaction(owner_id=env.operator_id, token_ids=[nft_token_id])
...
def test_integration_token_reject_transaction_fails_when_fungible_token_owner_has_no_balance():
    ...
    try:
        ...
        token_associate_transaction = TokenAssociateTransaction(account_id=account_id, token_ids=[token_id])
        ...
        token_reject_transaction = TokenRejectTransaction(owner_id=account_id, token_ids=[token_id])

        token_reject_transaction.freeze_with(env.client)
        token_reject_transaction.sign(new_account_private_key)
        receipt = token_reject_transaction.execute(env.client)

        assert receipt.status == ResponseCode.INSUFFICIENT_TOKEN_BALANCE, (
            f"Token rejection should have failed with INSUFFICIENT_TOKEN_BALANCE status but got: {ResponseCode(receipt.status).name}"
        )
    finally:
        env.close()
...
def test_integration_token_reject_transaction_fails_when_nft_owner_has_no_balance():
    ...
    try:
        ...
        # Mint NFTs
        mint_transaction = TokenMintTransaction(token_id=nft_token_id, metadata=[b"test metadata"])
        ...
        nft_associate_transaction = TokenAssociateTransaction(account_id=account_id, token_ids=[nft_token_id])
        ...
        nft_reject_transaction = TokenRejectTransaction(owner_id=account_id, nft_ids=[nft_id])
        ...
        assert receipt.status == ResponseCode.INVALID_OWNER_ID, (
            f"Token rejection should have failed with INVALID_OWNER_ID status but got: {ResponseCode(receipt.status).name}"
        )
    finally:
        env.close()
...
def test_integration_token_reject_transaction_fails_when_rejecting_nft_with_token_id():
    ...
    try:
        ...
        mint_transaction = TokenMintTransaction(
            token_id=nft_token_id, metadata=[b"metadata1", b"metadata2", b"metadata3"]
        )
        ...
        nft_transfer_transaction.add_nft_transfer(
            NftId(nft_token_id, serials[1]), env.client.operator_account_id, account_id
        )
        nft_transfer_transaction.add_nft_transfer(
            NftId(nft_token_id, serials[2]), env.client.operator_account_id, account_id
        )
        ...
        token_reject_transaction = TokenRejectTransaction(owner_id=account_id, token_ids=[nft_token_id])
        ...
        assert receipt.status == ResponseCode.ACCOUNT_AMOUNT_TRANSFERS_ONLY_ALLOWED_FOR_FUNGIBLE_COMMON, (
            f"Token rejection should have failed with ACCOUNT_AMOUNT_TRANSFERS_ONLY_ALLOWED_FOR_FUNGIBLE_COMMON status but got: {ResponseCode(receipt.status).name}"
        )
    finally:
        env.close()
...
def test_token_reject_transaction_fails_with_nft_token_frozen():
    ...
    try:
        ...
        # Mint NFTs
        mint_transaction = TokenMintTransaction(
            token_id=nft_token_id, metadata=[b"test metadata", b"test metadata", b"test metadata"]
        )
        ...
        # Associate the NFT to the receiver
        associate_transaction = TokenAssociateTransaction(account_id=account_id, token_ids=[nft_token_id])
        ...
        # Freeze the token
        token_freeze_transaction = TokenFreezeTransaction(token_id=nft_token_id, account_id=account_id)

        receipt = token_freeze_transaction.execute(env.client)

        assert receipt.status == ResponseCode.SUCCESS, (
            f"Token freeze failed with status: {ResponseCode(receipt.status).name}"
        )

        # Reject the NFT - should fail with ACCOUNT_FROZEN_FOR_TOKEN
        nft_reject_transaction = TokenRejectTransaction(owner_id=account_id, nft_ids=[NftId(nft_token_id, serials[1])])

        nft_reject_transaction.freeze_with(env.client)
        nft_reject_transaction.sign(new_account_private_key)
        receipt = nft_reject_transaction.execute(env.client)

        assert receipt.status == ResponseCode.ACCOUNT_FROZEN_FOR_TOKEN, (
            f"Token rejection should have failed with ACCOUNT_FROZEN_FOR_TOKEN status but got: {ResponseCode(receipt.status).name}"
        )
    finally:
        env.close()
...
def test_token_reject_transaction_fails_with_fungible_token_frozen():
    ...
    try:
        ...
        associate_transaction = TokenAssociateTransaction(account_id=account_id, token_ids=[token_id])
        ...
        # Freeze the token
        token_freeze_transaction = TokenFreezeTransaction(token_id=token_id, account_id=account_id)
...
def test_token_reject_transaction_receiver_sig_required_nft():
    ...
    try:
        ...
        receipt = (
            AccountCreateTransaction()
            .set_key_without_alias(treasury_public_key)
            .set_initial_balance(Hbar(0))
            .set_receiver_signature_required(True)
            .set_account_memo("Treasury Account")
            .freeze_with(env.client)
            .sign(treasury_private_key)
            .execute(env.client)
        )
        assert receipt.status == ResponseCode.SUCCESS, (
            f"Treasury account creation failed with status: {ResponseCode(receipt.status).name}"
        )
        ...
        receipt = (
            ...
            .set_metadata([b"test metadata 1", b"test metadata 2"])
            .execute(env.client)
        )
        assert receipt.status == ResponseCode.SUCCESS, (
            f"NFT minting failed with status: {ResponseCode(receipt.status).name}"
        ...
        assert receipt.status == ResponseCode.SUCCESS, (
            f"Receiver account creation failed with status: {ResponseCode(receipt.status).name}"
        )
        receiver_id = receipt.account_id

        associate_transaction = TokenAssociateTransaction(account_id=receiver_id, token_ids=[nft_token_id])
        ...
        assert receipt.status == ResponseCode.SUCCESS, (
            f"NFT rejection failed with status: {ResponseCode(receipt.status).name}"
        )
        ...
        assert token_balance.token_balances.get(nft_token_id) == 1, (
            f"Expected NFT balance to be 1, got {token_balance.token_balances.get(nft_token_id)}"
        )
...
def test_token_reject_transaction_receiver_sig_required_fungible():
    ...
    try:
        ...
        transaction = AccountCreateTransaction(
            key=treasury_public_key, initial_balance=Hbar(0), receiver_signature_required=True, memo="Treasury Account"
        )
        ...
        fungible_token_id = create_fungible_token(
            env,
            [
                lambda tx: tx.set_treasury_account_id(treasury_id).freeze_with(env.client),
                lambda tx: tx.sign(treasury_private_key),
        ...
        receiver_transaction = AccountCreateTransaction(
            key=receiver_public_key, initial_balance=Hbar(1), memo="Receiver Account"
        )
        ...
        # Associate the token to the receiver
        associate_transaction = TokenAssociateTransaction(account_id=receiver_id, token_ids=[fungible_token_id])
        receipt = associate_transaction.freeze_with(env.client).sign(receiver_private_key).execute(env.client)
        assert receipt.status == ResponseCode.SUCCESS, (
            f"Token association failed with status: {ResponseCode(receipt.status).name}"
        )
        ...
        receipt = (
            TokenRejectTransaction()
            .set_owner_id(receiver_id)
            .set_token_ids([fungible_token_id])
            .freeze_with(env.client)
            .sign(receiver_private_key)
            .execute(env.client)
        )
        ...
        assert receiver_balance.token_balances.get(fungible_token_id) == 0, (
            f"Expected token balance to be 0, got {receiver_balance.token_balances.get(fungible_token_id)}"
        )
        ...
        assert treasury_balance.token_balances.get(fungible_token_id) == 1000, (
            f"Expected treasury token balance to be 1000, got {treasury_balance.token_balances.get(fungible_token_id)}"
        )
    finally:
        env.close()
