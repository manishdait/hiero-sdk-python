...
def test_token_update_nfts_updates_metadata():
    ...
    try:
        ...
        nft_id = create_nft_token(
            env,
            [lambda tx: tx.set_supply_key(supply_private_key), lambda tx: tx.set_metadata_key(metadata_private_key)],
        )

        # Mint NFTs with initial metadata
        mint_transaction = TokenMintTransaction(token_id=nft_id, metadata=mint_metadata)
        ...
        update_transaction = TokenUpdateNftsTransaction(
            token_id=nft_id, serial_numbers=serials[: nft_count // 2], metadata=updated_metadata
        )

        receipt = update_transaction.freeze_with(env.client).sign(metadata_private_key).execute(env.client)
        assert receipt.status == ResponseCode.SUCCESS, (
            f"NFT update failed with status: {ResponseCode(receipt.status).name}"
        )
        ...
        assert nfts_metadata == mint_metadata[nft_count // 2 :], (
            f"Original metadata should be unchanged for serial {serial}"
        )
...
def test_can_update_empty_nft_metadata():
    ...
    try:
        ...
        update_transaction = TokenUpdateNftsTransaction(
            token_id=nft_id, serial_numbers=serials, metadata=updated_metadata
        )
...
def test_cannot_update_nft_metadata_when_key_is_not_set():
    ...
    try:
        ...
        assert receipt.status == ResponseCode.INVALID_SIGNATURE, (
            f"NFT update should fail with INVALID_SIGNATURE, got status: {ResponseCode(receipt.status).name}"
        )
    finally:
        env.close()
...
def test_cannot_update_nft_metadata_when_transaction_not_signed_with_metadata_key():
    ...
    try:
        ...
        tx = TokenUpdateNftsTransaction(token_id=nft_id, serial_numbers=serials, metadata=updated_metadata)
...
def test_token_update_nfts_transaction_fails_for_nonexistent_serial():
    ...
    try:
        ...
        assert receipt.status == ResponseCode.INVALID_NFT_ID, (
            f"NFT update should fail with INVALID_NFT_ID, got status: {ResponseCode(receipt.status).name}"
        )
    finally:
        env.close()
