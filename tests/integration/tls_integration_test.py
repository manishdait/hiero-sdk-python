...
def test_tls_can_be_disabled_manually():
    ...
    try:
        ...
        assert client.is_transport_security() is False, (
            "TLS should be disabled after calling set_transport_security(False)"
        )

        # Verify all nodes now use plaintext ports
        for node in network.nodes:
            assert node._address._is_transport_security() is False, (
                f"Node {node._account_id} should use plaintext port after disabling"
            )
            assert node._address._get_port() == 50211, f"Node {node._account_id} should use port 50211 for plaintext"
    finally:
        client.close()
...
def test_tls_query_execution_without_verification():
    ...
    try:
        ...
        if network_name in ("localhost", "solo", "local"):
            pytest.skip(
                f"TLS query execution test skipped for {network_name} network (TLS may not be properly configured)"
            )
