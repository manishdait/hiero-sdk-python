...
def test_integration_account_delete_transaction_can_execute(env):
    ...
    assert transfer_account_info.account_id == transfer_account.id, "Account ID should match"
    assert transfer_account_info.balance.to_tinybars() == Hbar(2).to_tinybars(), (
...
def test_integration_account_delete_transaction_fails_with_invalid_account_id(env):
    ...
    assert receipt.status == ResponseCode.INVALID_ACCOUNT_ID, (
        f"Delete account should have failed with INVALID_ACCOUNT_ID status but got: {ResponseCode(receipt.status).name}"
    )
...
def test_integration_account_delete_transaction_fails_with_no_signature(env):
    ...
    assert receipt.status == ResponseCode.INVALID_SIGNATURE, (
        f"Delete account should have failed with INVALID_SIGNATURE but got: {ResponseCode(receipt.status).name}"
    )
...
def test_integration_account_delete_transaction_fails_with_pending_airdrops(env):
    ...
    receipt = TokenMintTransaction().set_token_id(nft_id).set_metadata(nft_metadata).execute(env.client)
    assert receipt.status == ResponseCode.SUCCESS, f"Token mint failed: {ResponseCode(receipt.status).name}"
    ...
    assert receipt.status == ResponseCode.SUCCESS, f"Token airdrop failed: {ResponseCode(receipt.status).name}"
