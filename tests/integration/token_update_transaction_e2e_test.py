...
def test_integration_token_update_transaction_can_execute():
    ...
    try:
        ...
        assert receipt.status == ResponseCode.SUCCESS, (
            f"Token update transaction failed with status: {ResponseCode(receipt.status).name}"
        )
        ...
        assert info.freeze_key.to_bytes_raw() == private_key.public_key().to_bytes_raw(), (
            "Freeze key did not update correctly"
        )
        assert info.admin_key.to_bytes_raw() == env.public_operator_key.to_bytes_raw(), (
            "Admin key mismatch after update"
        )
    finally:
        env.close()
...
def test_integration_token_update_preserves_fields_without_updating_parameters():
    ...
    try:
        ...
        original_info = TokenInfoQuery().set_token_id(token_id).execute(env.client)

        receipt = TokenUpdateTransaction().set_token_id(token_id).execute(env.client)
        assert receipt.status == ResponseCode.SUCCESS, (
            f"Token update transaction failed with status: {ResponseCode(receipt.status).name}"
        )
        ...
        assert info.auto_renew_period == original_info.auto_renew_period, (
            "Token auto renew period should not have been changed"
        )
        assert info.auto_renew_account == original_info.auto_renew_account, (
            "Token auto renew account should not have been changed"
        )
        assert info.expiry == original_info.expiry, "Token expiry should not have been changed"
        assert info.admin_key.to_bytes_raw() == original_info.admin_key.to_bytes_raw(), (
            "Admin key should not have changed"
        )
        assert info.freeze_key.to_bytes_raw() == original_info.freeze_key.to_bytes_raw(), (
            "Freeze key should not have changed"
        )
        assert info.wipe_key.to_bytes_raw() == original_info.wipe_key.to_bytes_raw(), "Wipe key should not have changed"
        assert info.supply_key.to_bytes_raw() == original_info.supply_key.to_bytes_raw(), (
            "Supply key should not have changed"
        )
...
def test_integration_token_update_transaction_different_keys():
    ...
    try:
        ...
        tx = AccountCreateTransaction().set_key_without_alias(keys[0].public_key()).set_initial_balance(Hbar(2))
...
def test_integration_token_update_transaction_treasury():
    ...
    try:
        ...
        tx = TokenAssociateTransaction().set_account_id(account_id).add_token_id(token_id).freeze_with(env.client)
...
def test_integration_token_update_transaction_fail_invalid_token_id():
    ...
    try:
        ...
        assert receipt.status == ResponseCode.INVALID_TOKEN_ID, (
            f"Token update should have failed with INVALID_TOKEN_ID status but got: {ResponseCode(receipt.status).name}"
        )
    finally:
        env.close()
...
def test_integration_token_update_transaction_fungible_metadata():
    ...
    try:
        ...
        receipt = TokenUpdateTransaction().set_token_id(token_id).set_metadata(new_metadata).execute(env.client)
        assert receipt.status == ResponseCode.SUCCESS, (
            f"Token update failed with status: {ResponseCode(receipt.status).name}"
        )
...
def test_integration_token_update_transaction_metadata_immutable_fungible_token():
    ...
    try:
        ...
        token_id = create_fungible_token(
            env, opts=[lambda tx: tx.set_metadata_key(metadata_key), lambda tx: tx.set_admin_key(None)]
        )
...
def test_integration_token_update_transaction_metadata_immutable_nft():
    ...
    try:
        ...
        token_id = create_nft_token(
            env, [lambda tx: tx.set_metadata_key(metadata_key), lambda tx: tx.set_admin_key(None)]
        )
...
def test_token_update_transaction_cannot_update_metadata_fungible():
    ...
    try:
        ...
        receipt = TokenUpdateTransaction().set_token_id(token_id).set_token_memo("updated memo").execute(env.client)
        assert receipt.status == ResponseCode.SUCCESS, (
            f"Token update failed with status: {ResponseCode(receipt.status).name}"
        )
...
def test_integration_token_update_transaction_cannot_update_metadata_nft():
    ...
    try:
        ...
        # Try to update token with new memo, metadata should remain unchanged
        receipt = TokenUpdateTransaction().set_token_id(token_id).set_token_memo("asdf").execute(env.client)
        assert receipt.status == ResponseCode.SUCCESS, (
            f"Token update failed with status: {ResponseCode(receipt.status).name}"
        )
...
def test_integration_token_update_transaction_erase_metadata_fungible_token():
    ...
    try:
        ...
        receipt = TokenUpdateTransaction().set_token_id(token_id).set_metadata(b"").execute(env.client)
        assert receipt.status == ResponseCode.SUCCESS, (
            f"Token update failed with status: {ResponseCode(receipt.status).name}"
        )
...
def test_integration_token_update_transaction_cannot_update_metadata_without_key_fungible():
    ...
    try:
        ...
        receipt = TokenUpdateTransaction().set_token_id(token_id).set_metadata(b"New metadata").execute(env.client)

        assert receipt.status == ResponseCode.INVALID_SIGNATURE, (
            f"Token update should have failed with INVALID_SIGNATURE status but got: {ResponseCode(receipt.status).name}"
        )
    finally:
        env.close()
...
def test_integration_token_update_transaction_cannot_update_immutable_fungible_token():
    ...
    try:
        ...
        token_id = create_fungible_token(env, [lambda tx: tx.set_admin_key(None), lambda tx: tx.set_metadata_key(None)])

        # Try to update metadata on immutable token
        receipt = TokenUpdateTransaction().set_token_id(token_id).set_metadata(b"New metadata").execute(env.client)

        assert receipt.status == ResponseCode.TOKEN_IS_IMMUTABLE, (
            f"Token update should have failed with TOKEN_IS_IMMUTABLE status but got: {ResponseCode(receipt.status).name}"
        )
    finally:
        env.close()
...
def test_integration_token_update_transaction_cannot_update_immutable_nft():
    ...
    try:
        ...
        nft_token_id = create_nft_token(env, [lambda tx: tx.set_admin_key(None), lambda tx: tx.set_metadata_key(None)])

        # Try to update metadata on immutable token
        receipt = TokenUpdateTransaction().set_token_id(nft_token_id).set_metadata(b"New metadata").execute(env.client)
...
def test_integration_token_update_auto_renew_account():
    ...
    try:
        ...
        new_info = TokenInfoQuery().set_token_id(token_id).execute(env.client)
...
def test_integration_token_update_expiration_time():
    ...
    try:
        ...
        old_info = TokenInfoQuery().set_token_id(token_id).execute(env.client)
...
def test_integration_token_update_kyc_key_fungible_token():
    ...
    try:
        ...
        assert association_receipt.status == ResponseCode.SUCCESS, (
            f"Token association failed with status: {ResponseCode.get_name(association_receipt.status)}"
        )
        # Update Kyc Key
        ...
        assert kyc_receipt.status == ResponseCode.SUCCESS, (
            f"Token grant kyc failed with status: {ResponseCode.get_name(kyc_receipt.status)}"
        )
...
def test_integation_token_update_fee_schedule_key_fungible_token():
    ...
    try:
        ...
        token_info = TokenInfoQuery(token_id=token_id).execute(env.client)
        assert token_info.fee_schedule_key.to_string() == new_fee_schedule_key.public_key().to_string(), (
            "Updated fee_schedule_key mismatch"
        )
...
def test_integration_token_update_with_public_key():
    ...
    try:
        ...
        assert info.freeze_key.to_bytes_raw() == freeze_public_key.to_bytes_raw(), (
            "Freeze key on network should match the PublicKey used in transaction"
        )
    finally:
        env.close()
...
def test_integration_token_update_non_custodial_workflow():
    ...
    try:
        ...
        assert token_info.freeze_key.to_bytes_raw() == user_public_key.to_bytes_raw(), (
            "Freeze key on network should match the PublicKey used in transaction"
        )
...
def test_integration_token_update_with_ecdsa_public_key():
    ...
    try:
        ...
        assert token_info.admin_key.to_bytes_raw() == admin_public_key.to_bytes_raw(), (
            "Admin key on network should match the ECDSA PublicKey used in transaction"
        )
