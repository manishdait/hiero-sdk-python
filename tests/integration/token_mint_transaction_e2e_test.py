...
def test_integration_token_mint_nft_transaction_can_execute():
    ...
    try:
        ...
        mint_transaction = TokenMintTransaction(token_id=token_id, metadata=metadata)

        mint_transaction.freeze_with(env.client)
        receipt = mint_transaction.execute(env.client)

        assert receipt.status == ResponseCode.SUCCESS, (
            f"NFT token minting failed with status: {ResponseCode(receipt.status).name}"
        )
    finally:
        env.close()
...
def test_integration_token_mint_fungible_transaction_can_execute():
    ...
    try:
        ...
        mint_transaction = TokenMintTransaction(token_id=token_id, amount=2000)
