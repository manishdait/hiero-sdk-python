...
def test_integration_free_query_no_cost():
    ...
    try:
        ...
        balance_before = CryptoGetAccountBalanceQuery().set_account_id(account_id).execute(env.client)

        balance_result = query.execute(env.client)
        assert balance_result.hbars.to_tinybars() == initial_balance.to_tinybars()

        balance_after = CryptoGetAccountBalanceQuery().set_account_id(account_id).execute(env.client)
...
def test_integration_free_query_with_manual_payment():
    ...
    try:
        ...
        query = (
            CryptoGetAccountBalanceQuery().set_account_id(account_id).set_query_payment(Hbar(2))  # Set a high payment
        )
...
def test_integration_paid_query_payment_too_high_fails():
    ...
    try:
        ...
        payment = Hbar(2)
        query = TokenInfoQuery().set_token_id(token_id).set_query_payment(payment)
