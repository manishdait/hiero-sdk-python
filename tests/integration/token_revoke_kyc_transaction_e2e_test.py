...
def test_token_revoke_kyc_transaction_can_execute():
    ...
    try:
        ...
        receipt = TokenGrantKycTransaction().set_account_id(account_id).set_token_id(token_id).execute(env.client)
        assert receipt.status == ResponseCode.SUCCESS, (
            f"Token grant KYC failed with status: {ResponseCode(receipt.status).name}"
        )

        # Revoke KYC from the new account
        receipt = TokenRevokeKycTransaction().set_account_id(account_id).set_token_id(token_id).execute(env.client)
        assert receipt.status == ResponseCode.SUCCESS, (
...
def test_token_revoke_kyc_transaction_fails_with_no_kyc_key():
    ...
    try:
        ...
        receipt = TokenRevokeKycTransaction().set_account_id(account_id).set_token_id(token_id).execute(env.client)
        assert receipt.status == ResponseCode.TOKEN_HAS_NO_KYC_KEY, (
            f"Token revoke KYC should have failed with TOKEN_HAS_NO_KYC_KEY status but got: {ResponseCode(receipt.status).name}"
        )
...
def test_token_revoke_kyc_transaction_fails_when_account_not_associated():
    ...
    try:
        ...
        receipt = TokenRevokeKycTransaction().set_account_id(account_id).set_token_id(token_id).execute(env.client)
        assert receipt.status == ResponseCode.TOKEN_NOT_ASSOCIATED_TO_ACCOUNT, (
            f"Token revoke KYC should have failed with TOKEN_NOT_ASSOCIATED_TO_ACCOUNT status but got: {ResponseCode(receipt.status).name}"
        )
    finally:
        env.close()
