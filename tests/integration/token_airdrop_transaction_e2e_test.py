...
def _mint_nft(env: IntegrationTestEnv, nft_id):
    token_mint_tx = TokenMintTransaction(token_id=nft_id, metadata=[b"NFT Token A"])
...
def _associate_token_to_account(env: IntegrationTestEnv, account_id, private_key, token_ids):
    token_associate_tx = TokenAssociateTransaction(account_id=account_id, token_ids=token_ids)
...
def test_integration_token_airdrop_transaction_can_execute():
    ...
    try:
        ...
        airdrop_tx = TokenAirdropTransaction(
            ...
            nft_transfers=[TokenNftTransfer(nft_id, env.client.operator_account_id, new_account_id, serial_number)],
        )
        ...
        assert airdrop_receipt.status == ResponseCode.SUCCESS, (
            f"Token airdrop failed with status: {ResponseCode(airdrop_receipt.status).name}"
        )
