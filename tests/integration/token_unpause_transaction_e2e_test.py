...
def test_unpause_token(env, pausable_token):
    ...
    unpause_tx = TokenUnpauseTransaction().set_token_id(pausable_token).freeze_with(env.client).sign(pause_key)
...
def test_unpause_token_without_pause_key_signature(env, pausable_token):
    ...
    unpause_tx = TokenUnpauseTransaction().set_token_id(pausable_token).freeze_with(env.client)
...
def test_unpause_token_with_invalid_pasue_key(env, pausable_token):
    ...
    unpause_tx = (
        TokenUnpauseTransaction().set_token_id(pausable_token).freeze_with(env.client).sign(PrivateKey.generate())
    )
