...
Regular developers do not have the required permissions to create or update
...
def update_node(client, node_id, admin_key, account_id, node_account_pk):
    ...
    # Create and execute the node update transaction
    receipt = (
        NodeUpdateTransaction()
        .set_node_id(node_id)
        .set_account_id(account_id)
        .set_description(updated_description)
        .set_gossip_endpoints([updated_gossip_endpoint1, updated_gossip_endpoint2])
        .set_service_endpoints([updated_service_endpoint1, updated_service_endpoint2])
    ...
    # Confirm successful execution
    if receipt.status != ResponseCode.SUCCESS:
        print(f"Node update failed with status: {ResponseCode(receipt.status).name}")
        sys.exit(1)
