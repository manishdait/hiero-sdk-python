...
class Logger:
    ...
    def __init__(self, level: LogLevel | None = None, name: str | None = None) -> None:
        """
    ...
    def set_level(self, level: LogLevel | str) -> Logger:
        """
        Sets the current logging level for the SDK logger.

        Args:
            level (LogLevel | str): The new minimum log level. Can be a LogLevel enum or a string (e.g., "INFO").
    ...
    def _format_args(self, message: str, args: Sequence[object]) -> str:
        ...
        if not args or len(args) % 2 != 0:
            return message
        pairs = [f"{args[i]} = {args[i + 1]}" for i in range(0, len(args), 2)]
