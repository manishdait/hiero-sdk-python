...
class TokenAirdropTransaction(AbstractTokenTransferTransaction["TokenAirdropTransaction"]):
    ...
    def __init__(
        self, token_transfers: list[TokenTransfer] | None = None, nft_transfers: list[TokenNftTransfer] | None = None
    ) -> None:
        """
    ...
    def _from_proto(cls, proto: token_airdrop_pb2.TokenAirdropTransactionBody) -> TokenAirdropTransaction:
        ...
        for transfer in proto.token_transfers:
            if transfer.transfers:
                token_transfers.extend(TokenTransfer._from_proto(transfer))

            elif transfer.nftTransfers:
                nft_transfers.extend(TokenNftTransfer._from_proto(transfer))

        return cls(token_transfers=token_transfers, nft_transfers=nft_transfers)
    ...
    def _get_method(self, channel: _Channel) -> _Method:
        ...
        return _Method(transaction_func=token_service.airdropTokens, query_func=None)
