...
class AbstractTokenTransferTransaction(Transaction, ABC, Generic[T]):
    ...
    def _init_token_transfers(self, token_transfers: dict[TokenId, dict[AccountId, int]] | list[TokenTransfer]) -> None:
    ...
    def _init_nft_transfers(
        self, nft_transfers: dict[TokenId, list[tuple[AccountId, AccountId, int, bool]]] | list[TokenNftTransfer]
    ) -> None:
        ...
        Args:
            nft_transfers (dict[TokenId, list[tuple[AccountId, AccountId, int, bool]]] | list[TokenNftTransfer]):
        ...
        if isinstance(nft_transfers, list):
            ...
            for token_id, transfers in nft_transfers.items():
                for sender_id, receiver_id, serial_number, is_approved in transfers:
                    self._add_nft_transfer(token_id, sender_id, receiver_id, serial_number, is_approved)
        else:
            raise TypeError(
    ...
    def add_token_transfer(self: T, token_id: TokenId, account_id: AccountId, amount: int) -> T:
        """
    ...
    def add_token_transfer_with_decimals(
        self: T, token_id: TokenId, account_id: AccountId, amount: int, decimals: int
    ) -> T:
        """
    ...
    def add_approved_token_transfer(self: T, token_id: TokenId, account_id: AccountId, amount: int) -> T:
        """
    ...
    def add_nft_transfer(
        self: T, nft_id: NftId, sender_id: AccountId, receiver_id: AccountId, is_approved: bool = False
    ) -> T:
        """
    ...
    def add_approved_nft_transfer(self: T, nft_id: NftId, sender_id: AccountId, receiver_id: AccountId) -> T:
        """
    ...
    def build_token_transfers(self) -> list[basic_types_pb2.TokenTransferList]:
        ...
        for token_id, token_transfers in self.token_transfers.items():
            token_list = TokenTransferList(token=token_id, expected_decimals=token_transfers[0].expected_decimals)
